...
import { mkStdout, readApiVersion, readManifest, toAlphaNumWithSeparators, toTypeName, writeManifest, } from "./utils.js";
export function createScriptSkeleton({ manifest, scriptId, entryPoint, }) {
...
export async function addScript(id, { manifestPath: _manifestPath, scripts: _scripts, name, ...quiet }) {
    ...
    if (manifest.type !== "action") {
        throw new Error(`Mods of type ${manifest.type ?? "visualization"} do not support scripts.`);
    }
    if (manifest.scripts?.find((s) => s.id === id)) {
        throw new Error(`The mod already contains a script with id '${id}'.`);
    }
    if (existsSync(filePath)) {
        throw new Error(`The mod already contains a script file at '${filePath}'.`);
    }
    manifest.scripts = [
        ...(manifest.scripts ?? []),
        {
