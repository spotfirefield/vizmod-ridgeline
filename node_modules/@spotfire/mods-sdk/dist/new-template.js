...
import { ModType, capitalize, capitalizeBeforeSeparators, getDirname, getVersion, isModType, mkStdout, parseApiVersion, } from "./utils.js";
async function getTemplateFolder(type) {
...
export async function createTemplate(type, { outDir, apiVersion, gitignore, ...quiet }) {
...
export async function createGitIgnore({ targetFolder, ...quiet }) {
...
async function createModTemplate({ apiVersion: _apiVersion, modType, template, targetFolder, gitignore, ...quiet }) {
    const stdout = mkStdout(quiet);
    ...
    try {
        ...
        if (files.length > 0 && !quiet.quiet) {
            const wantToContinue = await ask(rl, "❓ The target folder is not empty. Are you sure you want to continue?");
            if (!wantToContinue) {
                return;
            }
        }
        ...
        try {
            ...
            throw new Error(`Could not copy templates folder to '${targetFolder}'.\nError: ${JSON.stringify(e)}`);
        }
        const apiVersion = parseApiVersion(_apiVersion ?? (modType === ModType.Action ? "2.0" : "1.3"));
        if (apiVersion.status === "error") {
            throw new Error(`Unregonized API version, error: ${apiVersion.error}`);
        }
        ...
        await writeFile(packageJsonpath, packageJson
            ...
            .replace("MODS-API-VERSION", apiVersion.result.toPackage()), { encoding: "utf-8" });
        ...
        await replaceInFile(manifestPath, (manifestTemplate) => {
            ...
            if (!apiVersion.result.supportsFeature("ModType")) {
                manifestJson = manifestJson.replace(`"type": "visualization",`, "");
            }
            return manifestJson
    ...
    async function replaceInFile(filePath, replaceFunction) {
...
async function ask(rl, question) {
    ...
    while (answer !== "yes" && answer !== "no") {
        ...
        if (answer !== "yes" && answer !== "no") {
            console.error(`Invalid answer '${answer}', please answer 'yes' or 'no'.`);
        }
    }
    return answer === "yes";
}
export function toModId(str) {
    const space = /\s\s*/g;
    const notValid = /[^A-z0-9- ]/g;
    return str.trim().toLowerCase().replace(notValid, "").replace(space, "-");
}
export function modIdToName(str) {
