...
import { ModType, features, formatIfPossible, formatVersion, getDirname, isParameterType, mkStdout, readApiVersion, toTypeName, typeFeature, } from "./utils.js";
...
export async function generateEnvFile({ manifest, envPath, ...quiet }) {
    ...
    if (apiVersionResult.status === "error") {
        error(`Failed to read apiVersion from manifest, ${envPath} could not be generated.`);
...
`;
        if (apiVersionResult.status === "success") {
            ...
            if (apiVersion.supportsFeature("Resources")) {
                const resourcesType = manifest.files != null
...
`;
                ...
                stdout(`Warning: The mod manifest contains resource files (specified under 'files' in the manifest) but targets an apiVersion earlier than ${formatVersion(features.Resources)}. These files will not be reachable from within the scripts in this action mod unless you increase your apiVersion.`);
            }
        }
        for (const param of script.parameters ?? []) {
            ...
            if (param.optional) {
                if (apiVersionResult.status === "success" &&
                    !apiVersionResult.result.supportsFeature("OptionalParameter")) {
                    error(`Parameter '${param.name}' is declared optional but the mod targets an apiVersion earlier than ${formatVersion(features.OptionalParameter)}. Consider targeting a later version to enable this feature.`);
                }
                tsType += `?`;
            }
            tsType += `: `;
            if (!param.type) {
                ...
                if (feature != null &&
                    ...
                    error(`Parameter '${param.name}' is type '${param.type}' but the mod targets an apiVersion earlier than ${formatVersion(features[feature])}. Consider targeting a later version to use this type.`);
                }
                if (param.enum) {
                    if (apiVersionResult.status === "success" &&
                        !apiVersionResult.result.supportsFeature("EnumParameter")) {
                        error(`Parameter '${param.name}' declares an enum but the mod targets an apiVersion earlier than ${formatVersion(features.EnumParameter)}. Consider targeting a later version to enable this feature.`);
                    }
                    ...
                    if (apiVersionResult.status === "success" &&
                        ...
                        error(`Parameter '${param.name}' declares array but the mod targets an apiVersion earlier than ${formatVersion(features.DataViews)}. Consider targeting a later version to enable this feature.`);
                    }
    ...
    if (!hasFailed) {
        if (envPath) {
            envFileContent = await formatIfPossible(envPath, envFileContent, true);
        }
        return {
            status: "success",
            result: envFileContent,
...
async function buildActionMod({ absOutDir, srcDir, manifestPath, esbuildConfig: esbuildConfigPath, envPath, debug, watch, ...quiet }) {
    ...
    const defaultConfig = {
        ...
        sourcemap: debug,
        entryNames: "[name]",
        entryPoints: [".js", ".ts"].map((extension) => path.join(scriptsDir, `*${extension}`)),
