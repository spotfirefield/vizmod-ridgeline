...
(async () => {
    ...
    program
        .command("new")
        .argument("<mod-type>", "what you want to create: action, visualization, or gitignore", (arg) => assertTemplateType(arg))
        .description("Create a new Mods project based on a template")
        .option("-o --out-dir <path>", "the folder in which the project should be created", ".")
        .option("--api-version <api-version>", "the Mods API version to use", (arg) => assertValidVersion(arg))
    ...
    program
        ...
        .option("--src <path>", "path to the folder containing the source files", "src")
        .option("--manifest-path <path>", "path to the mod-manifest.json file", "mod-manifest.json")
        ...
        .option("--esbuild-config", "path to a file which default exports an esbuild config", "esbuild.config.js")
        .quiet()
        .action(exec(build));
    program
        .command("add-script")
        ...
        .option("--scripts <path>", "path to the folder containing all scripts", "src/scripts")
        .option("--manifest-path <path>", "path to the mod-manifest.json file", "mod-manifest.json")
        .quiet()
        .action(exec(addScript));
    program
        .command("add-parameter")
        .argument("<script-id>", "the id of the script where the parameter should be added")
    ...
    function assertTemplateType(arg) {
        if (arg === "action") {
            ...
            program.error(`Invalid template type '${arg}'. Possible values are: action, visualization, or gitignore.`);
        }
    }
    function assertValidVersion(arg) {
        if (arg == null) {
            return undefined;
        }
        const result = parseApiVersion(arg);
        if (result.status === "error") {
            program.error(`Invalid api version argument, error: ${result.error}`);
        }
        else {
            return arg;
        }
